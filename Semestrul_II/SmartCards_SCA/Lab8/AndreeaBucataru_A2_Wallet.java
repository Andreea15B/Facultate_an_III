...
public class Wallet extends Applet {
    ...
    final static short SW_NEGATIVE_COUNT = 0x6A86;
    ...
    private Wallet(byte[] bArray, short bOffset, byte bLength) {
        ...
        count = 20; // nr tranzactii posibile
    ...
    private void debit(APDU apdu) {
        ...
        // check debit amount
        if ((debitAmount < 1 ) || (debitAmount > 4)) {
            ISOException.throwIt(SW_INVALID_TRANSACTION_AMOUNT);
        }
        
        // check >20 tranzactii
        if (count < 0) {
            ISOException.throwIt(SW_NEGATIVE_COUNT);
        }

        // 10 tranzactii - return 35%
        if (count == 10) {
        	ret = (debitAmount * 10) * 35 / 100;
        }
        ...
        balance = (short) (balance - debitAmount + ret);
        count -= 1; // scad nr tranzactii posibile
    }
